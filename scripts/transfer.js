$(function() {
	...
	function initPageStrings() {
		...
		$('.content_title').html('<img class="logo" src="images/logo.png">'+STRINGS.FILES_ON_DEVICE);
...
//		}
		...
		if (existFile.length > 0) {
			...
			if (existFile.parent().hasClass('progress_wrapper')) {
			    ...
			    var msg = STRINGS.FILE_EXISTS + "(" + fileName + ")";
				return msg;
			}
		}
		return null;
...
//			var msg = STRINGS.YOU_CHOOSE + totalFiles + STRINGS.CHOSEN_FILE_COUNT + actualFiles + STRINGS.VALID_CHOSEN_FILE_COUNT;
            var num = totalFiles - actualFiles;
            var fn = files[repeatIndex].name || files[repeatIndex].fileName;
            var msg = num + STRINGS.FILE_EXISTS_MULTI + fn;
			alert(msg);
		}
	}
